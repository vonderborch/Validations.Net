...
/// </summary>
public sealed class AndScope<T>
{
    ...
    /// <summary>
    /// Begins a rule chain for a member within this AND scope.
    /// </summary>
    public MemberRule<AndScope<T>, T, TMember> For<TMember>(
        ...
        [CallerArgumentExpression(nameof(selector))] string? selectorExpression = null)
    {
        ...
        return new MemberRule<AndScope<T>, T, TMember>(this, addStep, selector, memberPath);
    }

    /// <summary>
    /// Begins a rule chain that validates each element in a collection within this AND scope.
    /// </summary>
    public MemberRule<AndScope<T>, T, TMember> ForEach<TItem, TMember>(
        ...
        [CallerArgumentExpression(nameof(memberSelector))] string? memberExpression = null)
    {
        ...
        return new MemberRule<AndScope<T>, T, TMember>(this, addStep, extractor, $"{collectionPath}[].{memberPath}");
    }

    /// <summary>
    /// Begins a rule chain that validates each element in a collection directly within this AND scope.
    /// </summary>
    public MemberRule<AndScope<T>, T, TItem> ForEach<TItem>(
        Func<T, IEnumerable<TItem>> collectionSelector,
        Func<T, bool>? when = null,
        Func<T, bool>? unless = null,
        [CallerArgumentExpression(nameof(collectionSelector))] string? collectionExpression = null)
    {
        ...
        return new MemberRule<AndScope<T>, T, TItem>(this, addStep, extractor, $"{collectionPath}[]");
    }
    ...
    {
        _parent.AddStep(new AndRuleGroup<T>(_steps.ToArray()), ValidationSeverity.Error);
        return _parent;
    }
