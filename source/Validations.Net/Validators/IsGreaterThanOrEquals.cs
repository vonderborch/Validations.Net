...
public static class IsGreaterThanOrEquals
{
    ...
    /// <returns>
    ///     True if the value is greater than or equal to the comparison value; otherwise, false. Null values are
    ...
    /// <summary>
    ///     Validates that a value is greater than or equal to another value, throwing a <see cref="ValidationException" /> if
    ///     it isn't.
    /// </summary>
    /// <typeparam name="T">The type of the values to compare. Must implement <see cref="IComparable{T}" />.</typeparam>
    ...
    public static T ValidateIsGreaterThanOrEquals<T>(this T? value, T compareTo, string propertyName,
        Blackboard? blackboard = null) where T : IComparable<T>
    {
        if (!value.CheckIsGreaterThanOrEquals(compareTo))
        {
            throw new ValidationException("IsGreaterThanOrEquals", propertyName,
                $"{propertyName} must be greater than or equal to {compareTo}.", blackboard,
