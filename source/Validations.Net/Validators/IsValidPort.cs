...
public static class IsValidPort
{
    ...
    public static void EnsureIsValidDynamicPort(this int value, string fieldName, IBlackboard? blackboard)
    {
        ...
        if (!isValid)
        {
            ...
            throw ValidationException.Create(ValidatorName,
                $"The value must be a valid dynamic/private port number ({DynamicPortsMin}-{MaxPortNumber})", null,
                null, contextList);
        }
    }
    ...
    public static void EnsureIsValidNonPrivilegedPort(this int value, string fieldName, IBlackboard? blackboard)
    {
        ...
        if (!isValid)
        {
            ...
            throw ValidationException.Create(ValidatorName,
                $"The value must be a valid non-privileged port number ({RegisteredPortsMin}-{MaxPortNumber})", null,
                null, contextList);
        }
    }
    ...
    public static void EnsureIsValidPort(this int value, string fieldName, IBlackboard? blackboard)
    {
        ...
        if (!isValid)
        {
            ...
            throw ValidationException.Create(ValidatorName,
                $"The value must be a valid port number (0-{MaxPortNumber})", null, null, contextList);
        }
    }
    ...
    public static void EnsureIsValidPortInRange(this int value, string fieldName, IBlackboard? blackboard, int minPort,
        int maxPort)
    {
        ...
        if (!isValid)
        {
            ...
            throw ValidationException.Create(ValidatorName,
                $"The value must be a valid port number in range ({minPort}-{maxPort})", null, null, contextList);
        }
    }
    ...
    public static void EnsureIsValidRegisteredPort(this int value, string fieldName, IBlackboard? blackboard)
    {
        ...
        if (!isValid)
        {
            ...
            throw ValidationException.Create(ValidatorName,
                $"The value must be a valid registered port number ({RegisteredPortsMin}-{RegisteredPortsMax})", null,
                null, contextList);
        }
    }
    ...
    public static void EnsureIsValidWellKnownPort(this int value, string fieldName, IBlackboard? blackboard)
    {
        ...
        if (!isValid)
        {
            ...
            throw ValidationException.Create(ValidatorName,
                $"The value must be a valid well-known port number (0-{WellKnownPortsMax})", null, null, contextList);
        }
    }
    ...
    public static ValidationResult ValidateIsValidDynamicPort(this int value, string fieldName, IBlackboard? blackboard)
    {
        ...
        return isValid
            ...
            : ValidationResult.CreateFromValidationFailure(ValidatorName,
                $"The value must be a valid dynamic/private port number ({DynamicPortsMin}-{MaxPortNumber})", fieldName,
    ...
    /// <param name="blackboard">Optional blackboard for additional context.</param>
    /// <returns>A ValidationResult indicating success or failure.</returns>
    public static ValidationResult ValidateIsValidNonPrivilegedPort(this int value, string fieldName,
    ...
    public static ValidationResult ValidateIsValidPort(this int value, string fieldName, IBlackboard? blackboard)
    {
        ...
        return isValid
            ? ValidationResult.CreateFromValidationSuccess()
            : ValidationResult.CreateFromValidationFailure(ValidatorName,
                $"The value must be a valid port number (0-{MaxPortNumber})", fieldName, blackboard, contextList);
    }
    ...
    /// <param name="maxPort">The maximum port number (inclusive).</param>
    /// <returns>A ValidationResult indicating success or failure.</returns>
    public static ValidationResult ValidateIsValidPortInRange(this int value, string fieldName, IBlackboard? blackboard,
        int minPort, int maxPort)
    {
        ...
        return isValid
            ? ValidationResult.CreateFromValidationSuccess()
            : ValidationResult.CreateFromValidationFailure(ValidatorName,
                $"The value must be a valid port number in range ({minPort}-{maxPort})", fieldName, blackboard,
                contextList);
    }
    ...
    /// <param name="blackboard">Optional blackboard for additional context.</param>
    /// <returns>A ValidationResult indicating success or failure.</returns>
    public static ValidationResult ValidateIsValidRegisteredPort(this int value, string fieldName,
        IBlackboard? blackboard)
    {
        var isValid = CheckIsValidRegisteredPort(value);
        var contextList = new List<(string, object?)>
    ...
    /// <param name="blackboard">Optional blackboard for additional context.</param>
    /// <returns>A ValidationResult indicating success or failure.</returns>
    public static ValidationResult ValidateIsValidWellKnownPort(this int value, string fieldName,
        IBlackboard? blackboard)
    {
        var isValid = CheckIsValidWellKnownPort(value);
        var contextList = new List<(string, object?)>
        ...
        return isValid
            ? ValidationResult.CreateFromValidationSuccess()
            : ValidationResult.CreateFromValidationFailure(ValidatorName,
                $"The value must be a valid well-known port number (0-{WellKnownPortsMax})", fieldName, blackboard,
                contextList);
    }
}
