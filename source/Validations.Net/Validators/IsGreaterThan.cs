...
public static class IsGreaterThan
{
    ...
    /// <returns>
    ///     True if the value is greater than the comparison value; otherwise, false. Null values are considered less than
    ///     any non-null value.
    /// </returns>
    ...
    public static T ValidateIsGreaterThan<T>(this T? value, T? compareTo, string propertyName,
        Blackboard? blackboard = null) where T : IComparable<T>
    {
        if (!value.CheckIsGreaterThan(compareTo))
        {
            throw new ValidationException("IsGreaterThan", propertyName,
                $"{propertyName} must be greater than {compareTo}.", blackboard, new Dictionary<string, object?>
                {
                    { "value", value },
                    { "compareTo", compareTo }
                });
