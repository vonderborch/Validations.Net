...
public static class IsInRange
{
    ...
    public static bool CheckIsInRange<T>(this T value, T min, T max, bool minIsInclusive = true,
    ...
    /// <summary>
    ///     Validates that a numeric value is within a specified range, throwing a <see cref="ValidationException" /> if it
    ///     isn't.
    /// </summary>
    /// <typeparam name="T">The type of the numeric value. Must implement <see cref="INumber{T}" />.</typeparam>
    ...
    ///     Thrown when the value is not within the specified range or when min is greater
    ///     than max.
    /// </exception>
    public static T ValidateIsInRange<T>(this T value, T min, T max, string propertyName, bool minIsInclusive = true,
        bool maxIsInclusive = false, Blackboard? blackboard = null) where T : INumber<T>
    {
        ...
        {
            throw new ValidationException("IsInRange", propertyName,
                $"{propertyName} must be in the range [{min}, {max}].", blackboard, new Dictionary<string, object?>
                {
                    { "value", value },
                    { "min", min },
                    { "max", max }
