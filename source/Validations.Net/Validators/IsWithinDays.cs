...
public static class IsWithinDays
{
    ...
    public static void EnsureIsWithinDays(this DateTime value, int days, string? fieldName = null,
        IBlackboard? blackboard = null)
    {
        ...
        if (!isValid)
        {
            ...
            throw ValidationException.Create(ValidatorName,
                $"Value must be within {days} days from now. Actual difference: {actualDays:F2} days.", fieldName,
    ...
    /// <param name="blackboard">Optional blackboard for additional context.</param>
    /// <exception cref="ValidationException">Thrown when the value is not within the specified number of days from now.</exception>
    public static void EnsureIsWithinDays(this DateTimeOffset value, int days, string? fieldName = null,
        IBlackboard? blackboard = null)
    {
        var isValid = CheckIsWithinDays(value, days);
        if (!isValid)
    ...
    public static ValidationResult ValidateIsWithinDays(this DateTime value, int days, string? fieldName = null,
        IBlackboard? blackboard = null)
    {
        var isValid = CheckIsWithinDays(value, days);
        var now = DateTime.Now;
    ...
    /// <param name="blackboard">Optional blackboard for additional context.</param>
    /// <returns>A ValidationResult indicating whether the value is within the specified number of days from now.</returns>
    public static ValidationResult ValidateIsWithinDays(this DateTimeOffset value, int days, string? fieldName = null,
        IBlackboard? blackboard = null)
    {
        var isValid = CheckIsWithinDays(value, days);
        var now = DateTimeOffset.Now;
