...
public static class IsLessThanOrEquals
{
    ...
    /// <returns>
    ///     True if the value is less than or equal to the comparison value; otherwise, false. Null values are considered
    ...
    /// <summary>
    ///     Validates that a value is less than or equal to another value, throwing a <see cref="ValidationException" /> if it
    ///     isn't.
    /// </summary>
    /// <typeparam name="T">The type of the values to compare. Must implement <see cref="IComparable{T}" />.</typeparam>
    ...
    public static T ValidateIsLessThanOrEquals<T>(this T value, T compareTo, string propertyName,
        Blackboard? blackboard = null) where T : IComparable<T>
    {
        if (!value.CheckIsLessThanOrEquals(compareTo))
        {
            throw new ValidationException("ValidateIsLessThanOrEqualsAttribute", propertyName,
                $"{propertyName} must be less than or equal to {compareTo}.", blackboard,
