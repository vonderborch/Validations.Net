...
public static class IsLessThanOrEquals
{
    ...
    /// <returns>The value if it is less than or equal to the specified value, otherwise throws a <see cref="ValidationException"/>.</returns>
    /// <exception cref="ValidationException">Thrown when the value is not less than or equal to the specified value.</exception>
    public static T EnsureIsLessThanOrEquals<T>(this T value, T other, IBlackboard? blackboard = null, [CallerArgumentExpression(nameof(value))] string? parameterName = null) where T : IComparable<T>
    {
        ValidationResult result = value.ValidateIsLessThanOrEquals(other, blackboard, parameterName);
        if (!result.IsValid)
        {
    ...
    public static ValidationResult ValidateIsLessThanOrEquals<T>(this T value, T other, IBlackboard? blackboard = null, [CallerArgumentExpression(nameof(value))] string? parameterName = null) where T : IComparable<T>
    {
