...
public static class IsMoreThanXDaysInPast
{
    ...
    public static void EnsureIsMoreThanXDaysInPast(this DateTime value, int days, string? fieldName = null,
        IBlackboard? blackboard = null)
    {
        ...
        if (!isValid)
        {
            ...
            throw ValidationException.Create(ValidatorName,
                $"Value must be more than {days} days in the past. Actual difference: {actualDays:F2} days.", fieldName,
    ...
    /// <param name="blackboard">Optional blackboard for additional context.</param>
    /// <exception cref="ValidationException">Thrown when the value is not more than the specified number of days in the past.</exception>
    public static void EnsureIsMoreThanXDaysInPast(this DateTimeOffset value, int days, string? fieldName = null,
        IBlackboard? blackboard = null)
    {
        var isValid = CheckIsMoreThanXDaysInPast(value, days);
        if (!isValid)
