...
public static class IsNotToday
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static ValidationResult ValidateIsNotToday(this DateTime value, IBlackboard? blackboard = null, [CallerArgumentExpression(nameof(value))] string? fieldName = null)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static ValidationResult ValidateIsNotToday(this DateTimeOffset value, IBlackboard? blackboard = null, [CallerArgumentExpression(nameof(value))] string? fieldName = null)
    {
        if (CheckIsNotToday(value))
        {
            return ValidationResult.CreateFromValidationSuccess();
    ...
    public static DateTime EnsureIsNotToday(this DateTime value)
    {
        if (!CheckIsNotToday(value))
        {
            ...
            throw ValidationException.Create(ValidatorName, $"The date must not be today ({DateTime.Today:yyyy-MM-dd}). Actual date: {value:yyyy-MM-dd}", null, null, contextList);
        }
    ...
    public static DateTime? EnsureIsNotToday(this DateTime? value)
    {
        if (!CheckIsNotToday(value))
        {
            ...
            throw ValidationException.Create(ValidatorName, $"The date must not be today ({DateTime.Today:yyyy-MM-dd}). Actual date: {value?.ToString("yyyy-MM-dd") ?? "null"}", null, null, contextList);
        }
