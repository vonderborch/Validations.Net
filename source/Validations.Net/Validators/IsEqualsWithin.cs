...
/// <summary>
/// Provides extension methods for validating that numeric values are approximately equal to a specified value within a tolerance.
/// </summary>
public static class IsEqualsWithin
{
    ...
    /// <returns>True if the absolute difference between the values is less than or equal to the tolerance; otherwise, false. Two null values are considered equal.</returns>
    ...
    /// <summary>
    /// Validates that a numeric value is approximately equal to another value within a specified tolerance, throwing a <see cref="ValidationException"/> if it isn't.
    /// </summary>
    /// <typeparam name="T">The type of the numeric values to compare. Must implement <see cref="INumber{T}"/>.</typeparam>
    ...
    /// <exception cref="ValidationException">Thrown when the absolute difference between the values is greater than the tolerance.</exception>
