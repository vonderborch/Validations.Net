...
public static class IsNotEqualsWithTolerance
{
    ...
    /// If the values are equal within the tolerance, returns a <see cref="ValidationResult"/> containing validation failure details.
    /// </summary>
    /// <typeparam name="T">The type of the values to compare.</typeparam>
    ...
    public static ValidationResult ValidateIsNotEqualsWithTolerance<T>(this T value, T compareTo, T tolerance, string variableName,
    ...
    /// <summary>
    ///     Ensures that a value is not equal to another value within a specified tolerance, throwing a <see cref="ValidationException" /> if it is.
    /// </summary>
    /// <typeparam name="T">The type of the values to compare.</typeparam>
    /// <param name="value">The value to validate.</param>
    /// <param name="compareTo">The value to compare against.</param>
    ...
    public static T EnsureIsNotEqualsWithTolerance<T>(this T value, T compareTo, T tolerance, string propertyName,
        Blackboard? blackboard = null) where T : INumber<T>
    {
        ValidationResult result = value.ValidateIsNotEqualsWithTolerance(compareTo, tolerance, propertyName, blackboard);
        if (!result.IsValid)
        {
