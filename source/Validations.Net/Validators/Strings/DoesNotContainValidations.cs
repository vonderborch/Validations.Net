...
public static class DoesNotContainValidations
{
    ...
    /// <remarks>Note: Method name suggests it checks if a string does NOT contain a substring, but it currently returns true if it DOES contain it.</remarks>
    [DebuggerStepThrough]
    ...
    /// <remarks>Note: Method name suggests it checks if a string does NOT contain a character, but it currently returns true if it DOES contain it.</remarks>
    [DebuggerStepThrough]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool CheckDoesNotContain(this string value, char character, StringComparison comparisonType = StringComparison.Ordinal)
    {
        value.ValidateIsNotNull(nameof(value));
    ...
    /// <summary>
    /// Validates that the string does not contain the specified substring. Throws a ValidationException if the validation fails.
    /// </summary>
    ...
    /// <param name="comparisonType">The string comparison type to use. Defaults to StringComparison.Ordinal.</param>
    /// <param name="variableName">The name of the variable being validated, used in the exception message. Defaults to "value".</param>
    ...
    [DebuggerStepThrough]
    public static string ValidateDoesNotContain(this string value, string substring, StringComparison comparisonType = StringComparison.Ordinal, string variableName = "value", object? blackboard = null)
    {
        if (!value.CheckDoesNotContain(substring, comparisonType))
        {
            throw new ValidationException(variableName, $"{variableName} must not contain '{substring}'.", "DoesNotContain", blackboard, exceptionContext: new Dictionary<string, object?>
            {
                { "value", value },
                { "substring", substring }
            });
    ...
    /// <summary>
    /// Validates that the string does not contain the specified character. Throws a ValidationException if the validation fails.
    /// </summary>
    /// <param name="value">The string to validate. Cannot be null.</param>
    ...
    [DebuggerStepThrough]
    public static string ValidateDoesNotContain(this string value, char character, StringComparison comparisonType = StringComparison.Ordinal, string variableName = "value", object? blackboard = null)
    {
        if (!value.CheckDoesNotContain(character, comparisonType))
        {
            throw new ValidationException(variableName, $"{variableName} must not contain '{character}'.", "DoesNotContain", blackboard, exceptionContext: new Dictionary<string, object?>
            {
                { "value", value },
                { "character", character }
            });
