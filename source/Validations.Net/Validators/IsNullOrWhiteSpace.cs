...
public static class IsNullOrWhiteSpace
{
    ...
    /// <summary>
    ///     Validates that a string is null or consists only of whitespace characters, throwing a
    ...
    public static string ValidateIsNullOrWhiteSpace(this string? value, string propertyName,
        Blackboard? blackboard = null)
    {
        if (!value.CheckIsNullOrWhiteSpace())
        {
            throw new ValidationException("IsNullOrWhiteSpace", propertyName,
                $"{propertyName} must be null or whitespace.", blackboard, new Dictionary<string, object?>
                {
                    { "value", value }
                });
        }
