...
public static class IsNotInRange
{
    ...
    /// <returns>True if the value is not within the specified range; otherwise, false. Null values are considered not in range.</returns>
    ...
    /// <summary>
    /// Validates that a numeric value is not within a specified range, throwing a <see cref="ValidationException"/> if it is.
    /// </summary>
    /// <typeparam name="T">The type of the numeric value. Must implement <see cref="INumber{T}"/>.</typeparam>
    ...
    /// <exception cref="ValidationException">Thrown when the value is within the specified range or when min is greater than max.</exception>
