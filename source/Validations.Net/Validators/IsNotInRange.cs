...
public static class IsNotInRange
{
    ...
    public static bool CheckIsNotInRange<T>(this T value, T min, T max) where T : IComparable<T>
    {
    ...
    public static ValidationResult ValidateIsNotInRange<T>(this T value, T min, T max, string variableName,
        Blackboard? blackboard = null) where T : IComparable<T>
    {
        if (!value.CheckIsNotInRange(min, max))
        {
            ValidationResult result = new(
                new ValidationException("IsNotInRange", variableName,
                    $"{variableName} must not be between {min} and {max} (inclusive).",
    ...
    public static T EnsureIsNotInRange<T>(this T value, T min, T max, string propertyName,
        Blackboard? blackboard = null) where T : IComparable<T>
    {
        ValidationResult result = value.ValidateIsNotInRange(min, max, propertyName, blackboard);
        if (!result.IsValid)
        {
