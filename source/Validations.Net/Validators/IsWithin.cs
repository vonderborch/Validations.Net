...
public static class IsWithin
{
    ...
    public static bool CheckIsWithin<T>(this T? value, params T[] options)
    {
    ...
    /// <returns>True if the value is one of the specified options; otherwise, false.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool CheckIsWithin<T>(this T? value, ICollection<T> options)
    {
        foreach (T option in options)
        {
            if (value?.Equals(option) ?? false)
    ...
    public static T ValidateIsWithin<T>(this T? value, string propertyName, Blackboard? blackboard = null,
        params T[] options)
    {
        if (!value.CheckIsWithin(options))
        {
            throw new ValidationException("IsWithin", propertyName,
    ...
    /// <returns>The original value if validation succeeds.</returns>
    /// <exception cref="ValidationException">Thrown when the value is not one of the specified options.</exception>
    public static T ValidateIsWithin<T>(this T? value, ICollection<T> options, string propertyName,
