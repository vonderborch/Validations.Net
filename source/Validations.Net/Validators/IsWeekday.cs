...
public static class IsWeekday
{
    ...
    /// <summary>
    ///     Ensures that the specified nullable DateTime value falls on a weekday, throwing a ValidationException if it does
    ///     not.
    /// </summary>
    ...
    /// <param name="blackboard">Optional blackboard for additional context.</param>
    /// <exception cref="ValidationException">Thrown when the value does not fall on a weekday.</exception>
    public static void EnsureIsWeekday(this DateTimeOffset value, string? fieldName = null,
    ...
    /// <summary>
    ///     Ensures that the specified nullable DateTimeOffset value falls on a weekday, throwing a ValidationException if it
    ///     does not.
    /// </summary>
    ...
    public static ValidationResult ValidateIsWeekday(this DateTime value, string? fieldName = null,
        IBlackboard? blackboard = null)
    {
        var isValid = CheckIsWeekday(value);
        var contextList = new List<(string, object?)>
        ...
        return isValid
            ...
            : ValidationResult.CreateFromValidationFailure(ValidatorName,
                "Value must fall on a weekday (Monday through Friday).", fieldName, blackboard, contextList);
    }
    ...
    /// <param name="blackboard">Optional blackboard for additional context.</param>
    /// <returns>A ValidationResult indicating whether the value falls on a weekday.</returns>
    public static ValidationResult ValidateIsWeekday(this DateTimeOffset value, string? fieldName = null,
        IBlackboard? blackboard = null)
    {
        var isValid = CheckIsWeekday(value);
        var contextList = new List<(string, object?)>
