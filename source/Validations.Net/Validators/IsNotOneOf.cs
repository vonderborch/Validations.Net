...
public static class IsNotOneOf
{
    ...
    public static T ValidateIsNotOneOf<T>(this T? value, string propertyName, Blackboard? blackboard = null,
        params T[] options)
    {
        if (!value.CheckIsNotOneOf(options))
        {
            throw new ValidationException("IsNotOneOf", propertyName,
    ...
    /// <returns>The original value if validation succeeds.</returns>
    /// <exception cref="ValidationException">Thrown when the value is one of the specified options.</exception>
    public static T ValidateIsNotOneOf<T>(this T? value, ICollection<T> options, string propertyName,
