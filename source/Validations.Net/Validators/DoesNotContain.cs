...
public static class DoesNotContain
{
    ...
    {
        var result = value.ValidateDoesNotContain(substring, blackboard, validationFailureMessage, parameterName);
        if (!result.IsValid)
        {
    ...
    public static string? EnsureDoesNotContain(this string? value, string? substring, StringComparison comparisonType,
        ...
        [CallerArgumentExpression(nameof(value))] string? parameterName = null)
    {
        var result = value.ValidateDoesNotContain(substring, comparisonType, blackboard, validationFailureMessage, parameterName);
        if (!result.IsValid)
        {
            throw result.ValidationException!;
        }
    ...
    public static IEnumerable<T>? EnsureDoesNotContain<T>(this IEnumerable<T>? value, T? item,
        ...
        [CallerArgumentExpression(nameof(value))] string? parameterName = null)
    {
        var result = value.ValidateDoesNotContain(item, blackboard, validationFailureMessage, parameterName);
        if (!result.IsValid)
        {
            throw result.ValidationException!;
        }
