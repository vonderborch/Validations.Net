...
public sealed class InRangeValidator : IValidator
{
    ...
    {
        if (value is IComparable comparable && comparable.CheckIsInRange(this.Min, this.Max, this.MinInclusive, this.MaxInclusive))
            return ValidationResult.CreateFromValidationSuccess();
...
public sealed class InRangeValidator<T> : IValidator where T : IComparable<T>
{
    ...
    {
        this.Params = new InRangeParams<T>(min, max, minInclusive, maxInclusive);
    }
    ...
    public ValidationResult Validate(object? value, string? memberName = null, IBlackboard? blackboard = null)
    {
        if (value is IComparable comparable && comparable.CheckIsInRange(this.Params.Min, this.Params.Max, this.Params.MinInclusive, this.Params.MaxInclusive))
            return ValidationResult.CreateFromValidationSuccess();
        return ValidationResult.CreateFromValidationFailure(this.Name, this.DefaultFailureMessage, memberName, blackboard,
