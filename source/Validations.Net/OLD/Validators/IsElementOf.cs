...
public sealed class IsElementOfValidator : IValidator
{
    ...
    public ValidationResult Validate(object? value, string? memberName = null, IBlackboard? blackboard = null)
    {
        ...
        if (value is string str && this.AllowedValues.All(v => v is string))
        {
            var stringValues = this.AllowedValues.Cast<string>().ToArray();
            if (str.CheckIsElementOf(stringValues))
                return ValidationResult.CreateFromValidationSuccess();
        }
        else
        {
            if (((object?)value).CheckIsElementOf(this.AllowedValues))
                return ValidationResult.CreateFromValidationSuccess();
        }
    ...
    private ValidationResult Fail(object? value, string? memberName, IBlackboard? blackboard)
        => ValidationResult.CreateFromValidationFailure(this.Name, this.DefaultFailureMessage, memberName, blackboard,
            [("value", value), ("allowedValues", this.AllowedValues)]);
}
