...
public sealed class FileExistsValidator : IValidator
{
    ...
    public ValidationResult Validate(object? value, string? memberName = null, IBlackboard? blackboard = null)
    {
        if (value is string path)
            return path.ValidateDoesFileExist(blackboard, this.DefaultFailureMessage, memberName);
...
public sealed class ValidateDoesFileExistAttribute() : ValidationAttribute(DoesFileExist.ValidatorName)
{
    ...
    {
        var message = this.Message ?? DoesFileExist.DefaultValidationFailureMessage;
