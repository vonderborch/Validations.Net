...
public sealed class ValidAgeValidator : IValidator
{
    ...
    {
        if (value is int age)
            return age.ValidateIsValidAge(blackboard, this.DefaultFailureMessage, memberName);
        return ValidationResult.CreateFromValidationFailure(this.Name, "Value is not an int", memberName, blackboard,
            [("value", value)]);
    }
}
...
public sealed class ValidateIsValidAgeAttribute() : ValidationAttribute(IsValidAge.ValidatorName)
{
    ...
    {
        var message = this.Message ?? IsValidAge.DefaultValidationFailureMessage;
