...
public sealed class MinorValidator : IValidator
{
    ...
    {
        this.Params = new MinorParams(adultAge);
    }
    ...
    public ValidationResult Validate(object? value, string? memberName = null, IBlackboard? blackboard = null)
    {
        return value switch
        {
            int age when age.CheckIsMinor(this.Params.AdultAge) => ValidationResult.CreateFromValidationSuccess(),
            ...
            System.DateTime dob when dob.CheckIsMinor(this.Params.AdultAge) => ValidationResult.CreateFromValidationSuccess(),
            ...
            DateTimeOffset dob when dob.CheckIsMinor(this.Params.AdultAge) => ValidationResult.CreateFromValidationSuccess(),
...
public sealed class ValidateIsMinorAttribute(int adultAge = 18) : ValidationAttribute(IsMinor.ValidatorName)
{
    ...
    {
        if (value is int age)
            return age.CheckIsMinor(this._adultAge) ? ValidationResult.CreateFromValidationSuccess() : Fail(age, memberName, blackboard);
        if (value is System.DateTime dt)
            return dt.CheckIsMinor(this._adultAge) ? ValidationResult.CreateFromValidationSuccess() : Fail(dt, memberName, blackboard);
        if (value is DateTimeOffset dto)
            return dto.CheckIsMinor(this._adultAge) ? ValidationResult.CreateFromValidationSuccess() : Fail(dto, memberName, blackboard);
    ...
    {
        var message = this.Message ?? IsMinor.DefaultValidationFailureMessage;
