...
public sealed class IsNotElementOfValidator : IValidator
{
    ...
    {
        if (this.DisallowedValues.Length == 0)
            return ValidationResult.CreateFromValidationSuccess();

        if (value is string str && this.DisallowedValues.All(v => v is string))
        {
            var stringValues = this.DisallowedValues.Cast<string>().ToArray();
            if (str.CheckIsNotElementOf(stringValues))
                return ValidationResult.CreateFromValidationSuccess();
        }
        else
        {
            if (((object?)value).CheckIsNotElementOf(this.DisallowedValues))
                return ValidationResult.CreateFromValidationSuccess();
        }

        return ValidationResult.CreateFromValidationFailure(this.Name, this.DefaultFailureMessage, memberName, blackboard,
            [("value", value), ("disallowedValues", this.DisallowedValues)]);
    }
}
