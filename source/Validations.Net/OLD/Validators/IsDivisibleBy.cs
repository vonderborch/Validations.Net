...
public sealed class IsDivisibleByValidator : IValidator
{
    ...
    {
        this.Params = new IsDivisibleByParams(divisor);
    }
    ...
    {
        if (value is null || this.Params.Divisor == 0)
            return ValidationResult.CreateFromValidationFailure(this.Name, this.DefaultFailureMessage, memberName, blackboard,
                [("value", value), ("divisor", this.Params.Divisor)]);
        if (value is long l && l.CheckIsDivisibleBy(this.Params.Divisor))
            return ValidationResult.CreateFromValidationSuccess();
        if (value is int i && ((long)i).CheckIsDivisibleBy(this.Params.Divisor))
            return ValidationResult.CreateFromValidationSuccess();
        try
        {
            long converted = Convert.ToInt64(value);
            if (converted.CheckIsDivisibleBy(this.Params.Divisor))
                return ValidationResult.CreateFromValidationSuccess();
        }
