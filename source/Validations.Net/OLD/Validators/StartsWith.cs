...
public sealed class StartsWithValidator : IValidator
{
    ...
    {
        this.Params = new StartsWithParams(prefix, comparison);
    }
    ...
    {
        if (value is string s && s.CheckStartsWith(this.Params.Prefix, this.Params.Comparison))
            return ValidationResult.CreateFromValidationSuccess();
        return ValidationResult.CreateFromValidationFailure(this.Name, this.DefaultFailureMessage, memberName, blackboard,
            [("value", value), ("prefix", this.Params.Prefix)]);
    }
}
