...
public abstract class AbstractValidator<T> : IValidateWithContext<T>
{
    ...
    protected MemberRule<Validator<T>, T, TMember> For<TMember>(
        ...
        => this._validator.For(selector, when, unless, selectorExpression);
    ...
    protected MemberRule<Validator<T>, T, TMember> ForEach<TItem, TMember>(
        ...
        => this._validator.ForEach(collectionSelector, memberSelector, when, unless, collectionExpression, memberExpression);
    ...
    protected MemberRule<Validator<T>, T, TItem> ForEach<TItem>(
        ...
        Func<T, bool>? unless = null,
        [CallerArgumentExpression(nameof(collectionSelector))] string? collectionExpression = null)
        => this._validator.ForEach(collectionSelector, when, unless, collectionExpression);

    /// <summary>
    /// Opens a conditional scope.
    /// </summary>
    protected WhenScope<T> When(Func<T, bool> condition) => this._validator.When(condition);

    /// <summary>
    /// Opens an inverse conditional scope. Rules execute when the condition is false.
    /// </summary>
    protected WhenScope<T> Unless(Func<T, bool> condition) => this._validator.Unless(condition);

    /// <summary>
    /// Opens an OR scope.
    /// </summary>
    protected OrScope<T> Or() => this._validator.Or();

    /// <summary>
    /// Opens an AND scope.
    /// </summary>
    protected AndScope<T> And() => this._validator.And();
    ...
    protected Validator<T> Must(Func<T, bool> predicate, string failureMessage,
        ...
        => this._validator.Must(predicate, failureMessage, severity);
    ...
    protected Validator<T> MustAsync(Func<T, CancellationToken, Task<bool>> predicate,
        ...
        => this._validator.MustAsync(predicate, failureMessage, severity);

    /// <summary>
    /// Includes all rules from another <see cref="AbstractValidator{T}"/> as a composite step.
    /// </summary>
    protected void Include(AbstractValidator<T> other) => this._validator.Include(other);

    /// <summary>
    /// Includes all rules from a <see cref="Validator{T}"/> as a composite step.
    /// </summary>
    protected void Include(Validator<T> other) => this._validator.Include(other);
    ...
    public T Ensure(T value, IBlackboard? blackboard = null,
        ...
        => this._validator.Ensure(value, blackboard, validationFailureMessage, parameterName);
    ...
    public Task<ValidationResult> ValidateAsync(T value, IBlackboard? blackboard = null,
        ...
        => this._validator.ValidateAsync(value, blackboard, cancellationToken);
    ...
    public Task<bool> CheckAsync(T value, IBlackboard? blackboard = null,
        CancellationToken cancellationToken = default)
        => this._validator.CheckAsync(value, blackboard, cancellationToken);
    ...
    public Task<T> EnsureAsync(T value, IBlackboard? blackboard = null,
        string validationFailureMessage = "Validation failed",
        CancellationToken cancellationToken = default)
        => this._validator.EnsureAsync(value, blackboard, validationFailureMessage, cancellationToken);

    /// <summary>
    /// Builds an immutable <see cref="ValidationSet{T}"/> from the configured rules.
    /// </summary>
    public ValidationSet<T> Build() => this._validator.Build();
