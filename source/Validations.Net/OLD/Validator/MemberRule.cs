...
public sealed class MemberRule<TParent, T, TMember>
{
    ...
    {
        this._parent = parent;
        this._parentAddStep = addStep;
        this._addStep = addStep;
        this._selector = selector;
        this._memberPath = memberPath;
        this._lastTargetList = targetList;
    }
        ...
        List<(IValidationStep<T> Step, ValidationSeverity Severity)>? targetList = null)
    {
        ...
        this._parentAddStep = addStep;
        this._addStep = addStep;
        this._selector = _ => default!;
        this._memberPath = memberPath;
        this._forEachExtractor = forEachExtractor;
        this._lastTargetList = targetList;
    }

    internal Func<T, TMember> Selector => this._selector;
    internal string? MemberPath => this._memberPath;
    ...
    {
        if (this._forEachExtractor is not null)
            return ApplyForEach(validator, severity, message);

        var selector = this._selector;
        var memberPath = this._memberPath;
        this._addStep(new DelegateValidationStep<T>((value, bb) =>
        {
    ...
    {
        var extractor = this._forEachExtractor!;
        this._addStep(new DelegateAggregateValidationStep<T>((value, bb) =>
        {
        ...
        ValidationSeverity severity)
    {
        ...
        var selector = this._selector;
        var memberPath = this._memberPath ?? string.Empty;
        this._addStep(new DelegateAggregateValidationStep<T>((value, bb) =>
        {
            var member = selector(value);
        ...
        ValidationSeverity severity = ValidationSeverity.Error, string? message = null)
    {
        ...
        var selector = this._selector;
        var memberPath = this._memberPath;
        this._addStep(new AsyncDelegateValidationStep<T>(async (value, bb, ct) =>
        {
            var member = selector(value);
    ...
    {
        var extractor = this._forEachExtractor!;
        this._addStep(new AsyncDelegateAggregateValidationStep<T>(async (value, bb, ct) =>
        {
            var builder = ValidationResult.CreateBuilder();
            foreach (var (member, path) in extractor(value))
            {
    ...
    {
        this._cascadeSteps = new();
        this._lastTargetList = this._cascadeSteps;
        this._addStep = (step, severity) => this._cascadeSteps.Add((step, severity));
        return this;
    }
    ...
    {
        if (this._lastTargetList is { Count: > 0 })
        {
            var (step, severity) = this._lastTargetList[^1];
            this._lastTargetList[^1] = (new MessageOverrideValidationStep<T>(step, message, this._memberPath), severity);
        }
        return this;
    }
    ...
    public MemberRule<TParent, T, TMember> WithSeverity(ValidationSeverity severity)
    {
        if (this._lastTargetList is { Count: > 0 })
        {
            var (step, _) = this._lastTargetList[^1];
            this._lastTargetList[^1] = (step, severity);
        }
        return this;
    }
    ...
    public TParent End()
    {
        if (this._cascadeSteps is { Count: > 0 })
            this._parentAddStep(new CascadeValidationStep<T>(this._cascadeSteps.ToArray()), ValidationSeverity.Error);
        return this._parent;
    }
}
