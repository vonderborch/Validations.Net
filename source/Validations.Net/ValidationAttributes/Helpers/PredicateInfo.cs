...
/// <param name="Group">Optional group name that can categorize related predicates. May be null if the predicate is not grouped.</param>
    ...
    Type DeclaringType)
{
    ...
    /// of the static member types, such as <see cref="PredicateType.StaticField"/>, <see cref="PredicateType.StaticProperty"/>,
    ...
    /// <param name="group">An optional group name used to categorize the predicate. If null, the key will be based only on the name.</param>
    /// <returns>A string representing the unique key for the predicate, which combines the group and name if a group is provided.</returns>
    public static string GetKey(string name, string? group)
    {
        string key = group is null ? name : $"{group}-{name}";
        return key;
    }
}
