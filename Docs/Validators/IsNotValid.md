...
Checks for if a class or struct is not valid, used with `ValidationAttributes` attached to a class or struct's fields or
properties

## Available Extension Methods

- `CheckIsNotValid`
- `GetValidationResultForIsNotValid`
- `ValidateIsNotValid`
...
var result = person.CheckIsNotValid();
...
Person person = new("John", 19);
var result = person.GetValidationResultForIsNotValid(nameof(person));
person.ValidateIsNotValid(nameof(person));
